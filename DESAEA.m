function Offspring = DESAEA(Problem,Population,NN,k,beta,Na)
...
% This function is written by Jianqing Lin
    ...
    [Population,~,~] = EnvironmentalSelection(Population,Na);

    %% Parameter setting     
    PopDec      = Population.decs;
    [N,D]       = size(PopDec);
    
    %% Non-dominated Sorting & CrowdingDistance
    [FrontNo,MaxFN]  = NDSort(Population.objs,N);
    CrowdDis         = CrowdingDistance_func(Population.objs);
    
    %% Solution Selection
    [~,index_NSCD]   = sortrows([FrontNo;CrowdDis]',[1,-2]);
    Offspring_D      = PopDec(index_NSCD(1:k),:);

    %% EDA Modeling
    Model          = EDA(PopDec(FrontNo == 1,:),Problem.M);
    Model2         = EDA(PopDec(FrontNo == MaxFN,:),Problem.M);
    Model_Dif      = Model.mean-Model2.mean; 
    
    Model_Dif_sort = sort(abs(Model_Dif),'descend');    
    Index_dif = find(abs(Model_Dif) >= Model_Dif_sort(ceil(beta*D)));
    Index_dif2 = find(abs(Model_Dif) < Model_Dif_sort(ceil(beta*D)));

    %% Decomposition d
    PopDec_d   = PopDec(:,Index_dif);
    
    %% Build RBF-Surrogate 
    RBF_para = cell(1,Problem.M);
    Objs_Surrogate  = Population.objs;
    for i = 1:Problem.M
        RBF_para{i} = RBFCreate(PopDec_d, Objs_Surrogate(:,i), 'cubic');
    end
   
    %% Decomposition  D
    PopDec_D   = PopDec(:,Index_dif2);
    
    %% Build RBF-Surrogate 
    RBF_para_D = cell(1,Problem.M);
    Objs_Surrogate  = Population.objs;
    for i = 1:Problem.M
        RBF_para_D{i} = RBFCreate(PopDec_D, Objs_Surrogate(:,i), 'cubic');
    end
    
    %% Reproduction
    % EnvironmentalSelection
    [Population,~,~]  = EnvironmentalSelection(Population,NN);
    PopDec          = Population.decs;
    Pop_Surrogate   = surrogate_individual(PopDec(:,Index_dif),Population.objs);
    Offspring_d     = Reproduction1(Problem,Pop_Surrogate,RBF_para,Index_dif);
   
   
    %% Reproduction
    PopDec          = Population.decs;
    Pop_Surrogate   = surrogate_individual(PopDec(:,Index_dif2),Population.objs);
    Offspring_d2     = Reproduction1(Problem,Pop_Surrogate,RBF_para_D,Index_dif2);
     
    %% Replacement
    [Offspring_d,~,~]        = EnvironmentalSelection(Offspring_d,k);
    [Offspring_d2,~,~]       = EnvironmentalSelection(Offspring_d2,k);
    
    Offspring_D(:,Index_dif)  = Offspring_d.decs;
    Offspring_D(:,Index_dif2) = Offspring_d2.decs;
...
function CrowdDis = CrowdingDistance_func(PopObj)
...
function Offspring = Reproduction1(Problem,Particle,RBF_para,Index_dif)
        
    [proM,disM] = deal(ceil(Problem.D/100),20);
    ...
    for gen = 1 : 50
      ...
      %% Particle swarm optimization
        ...
        Off_Objs  = RBF_Predictor(OffDec, RBF_para, M);
    ...
    %% Polynomial mutation
    if Problem.FE >  0.85 * Problem.maxFE
...
function [Gbest,CrowdDis] = UpdateGbest(Gbest,N)
    ...
    CrowdDis = CrowdingDistance_func(Gbest.objs);
...
function Model = EDA(PopDec,M)
    ...
    for iter = 1 : 50
        ...
        [eVector,eValue] = eig(cov(PopDec-repmat(Model.mean,N,1)));
